...
const Drinks = ({ configurationId, width, height }) => {
    ...
    const handleDrinkCancel = () => {
        setShowElemAdd(false);
    };
    const handleDrinkCreate = () => {
        setUpdate(update + 1)
        setShowElemAdd(false);
    };
    ...
    return (
        <div className={'content'}>
                    ...
                    <div className={'game-list'}>
                        {drinks.map(drink => (
                            <div key={drink.id} className={'game'}
                                 onMouseEnter={() => handleMouseEnter(drink.id)}
            ...
            <div className={`screen-configuration ${!configurationId && ('configuration-disable')}`}>
                ...
                {showElemAdd && (
                    <DrinkCreate configurationId={configurationId} onDrinkCancel={handleDrinkCancel}
                                   onDrinkCreate={handleDrinkCreate}/>
                )}
                {showElemEdit && (
                    <GoodEdit onGoodEditCancel={handleDrinkEditCancel} onGoodEdit={handleDrinkEdit}
